...
# Примечание. Программа должна добавлять нужное количество пробелов, если название товара имеет меньшую длину, чем другие.
...
for obj in sorted(data.items()):
    max_len = len(max(data.keys(), key=len))
    prod, val = obj
    price = sum(ord(i) for i in prod if i.isalpha())
    print(f'{prod:{max_len}}: {price} UC x {val} = {price * val} UC')