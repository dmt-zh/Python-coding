# Объявите класс-декоратор InputValues с параметром render - функция или объект для преобразования данных из
# строк в другой тип данных. Чтобы реализовать такой декоратор в инициализаторе __init__() следует указать параметр render,
...
# Например, при вводе строки: "1 -5.3 0.34 abc 45f -5"
# должен возвращаться список: [1, None, None, None, None, -5]
...
class InputValues:
    ...
    def __call__(self, func):
        def wrapper(*args, **kwargs):
            get_input = func().split()
            return list(map(self.__render, get_input))
        return wrapper
...
class RenderDigit:
    def __call__(self, value, *args, **kwargs):
        result = value if (value.startswith('-') and value[1:].isdigit()) or value.isdigit() else None
