...
class Protists:
    def __init__(self, name=None, weight=None, age=None):
        self.name = name
        self.weight = weight
        self.old = age
...
def get_object(arr, class_name):
    return [obj for obj in arr if isinstance(obj, class_name)]

lst_animals = get_object(lst_objs, Animals)
lst_plants = get_object(lst_objs, Plants)
lst_mammals = get_object(lst_objs, Mammals)