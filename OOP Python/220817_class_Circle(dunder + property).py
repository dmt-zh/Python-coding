...
# circle = Circle(x, y, radius) где x, y - координаты центра окружности; radius - радиус окружности
...
class Circle:
    valid_attrs = {'x': (int, float), 'y': (int, float), 'radius': (int, float)}
    ...
    def __setattr__(self, key, value):
        if key in self.valid_attrs and type(value) not in self.valid_attrs[key]:
            raise TypeError("Неверный тип присваиваемых данных.")
