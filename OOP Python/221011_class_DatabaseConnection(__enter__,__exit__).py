# Вам поручено разработать класс DatabaseConnection для управления подключением к базе данных.
# Объекты этого класса создаются командой: conn = DatabaseConnection()

# В самом классе необходимо объявить метод:
# def connect(self, login, password): ...
# для подключения к БД. В данной реализации этот метод должен устанавливать локальный атрибут _fl_connection_open
# в значение True: _fl_connection_open = True
# и генерировать исключение с помощью собственного класса ConnectionError унаследованного от базового класса
# Exception.
