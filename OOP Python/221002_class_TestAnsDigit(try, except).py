# Вы начинаете разрабатывать свой сервис по тестированию. Для этого вам поручается разработать базовый
# класс Test для всех видов тестов, объекты которого создаются командой: test = Test(descr)
# где descr - формулировка теста (строка). Если длина строки descr меньше 10 или больше 10 000 символов,
# то генерировать исключение командой: raise ValueError('формулировка теста должна быть от 10 до 10 000 символов')
...
class Test:
    ...
    def __setattr__(self, key, value):
        if key in ('_descr',) and not 10 <= len(value) <= 10000:
...
class TestAnsDigit(Test):
    ...
    def __setattr__(self, key, value):
        if key in ('_ans_digit', '_max_error_digit') and type(value) not in (int, float) \
                or key in ('_max_error_digit',) and value < 0:
