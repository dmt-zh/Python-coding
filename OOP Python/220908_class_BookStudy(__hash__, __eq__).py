...
# year - год издания (целое число). Такие же публичные локальные атрибуты должны быть в объектах 
# класса BookStudy.
...
# После этого определить число книг с уникальными хэшами. Это число сохранить через переменную 
# unique_books (целое число).
...
for row in lst_in:
    name, author, year = list(map(str.strip, row.split(';')))
