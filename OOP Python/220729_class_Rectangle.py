...
# должны формироваться следующие локальные свойства: __x, __y - координаты точки на плоскости;
# и один геттер:
...
# r1 = Rectangle(Point(x1, y1), Point(x2, y2)) или r2 = Rectangle(x1, y1, x2, y2)
...
class Point:
    ...
    def __check_coords(cls, x, y):
        return type(x) in (float, int) and type(y) in (float, int)
